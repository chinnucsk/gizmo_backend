...
test_active_sessions(_) ->
    ...
    meck:expect(session_counter_api, active_sessions, fun(_,_,_) -> {ok, 5} end),
    ...
    [{_, {session_counter_api, active_sessions, [<<"key">>, undefined, undefined]}, {ok, 5}}] = meck:history(session_counter_api),
    meck:validate(application_obj),
    meck:validate(session_counter_api),
...
test_active_sessions_history(_) ->
    ...
    meck:expect(application_obj, exists, fun(_) -> true end),
    meck:new(session_counter_api),
    meck:expect(session_counter_api, active_sessions, fun(_,_,_) -> {ok, [{struct, [{<<"123">>, 1}]}, {struct, [{<<"456">>, 2}]}]} end),
    {ok, {{"HTTP/1.1", 200, "OK"}, Header, Body}} = httpc:request("http://127.0.0.1:8888/application/key/active_sessions"),
    "application/json" = proplists:get_value("content-type", Header),
    {struct, [{<<"active_sessions">>, [{struct, [{<<"123">>, 1}]}, {struct, [{<<"456">>, 2}]}]}]} = mochijson2:decode(Body),
    [{_, {application_obj, exists, [<<"key">>]}, true}] = meck:history(application_obj),
    [{_, {session_counter_api, active_sessions, [<<"key">>, undefined, undefined]}, {ok, [{struct, [{<<"123">>, 1}]}, {struct, [{<<"456">>, 2}]}]}}] = meck:history(session_counter_api),
    meck:validate(application_obj),
    meck:validate(session_counter_api),
    meck:unload(application_obj),
    meck:unload(session_counter_api).
...
test_active_sessions_error(_) ->
    ...
    meck:expect(application_obj, exists, fun(_) -> true end),
    meck:new(session_counter_api),
    meck:expect(session_counter_api, active_sessions, fun(_,_,_) -> {error, reason} end),
    ...
    [{_, {application_obj, exists, [<<"key">>]}, true}] = meck:history(application_obj),
    [{_, {session_counter_api, active_sessions, [<<"key">>, undefined, undefined]}, {error, reason}}] = meck:history(session_counter_api),
    meck:validate(application_obj),
    meck:validate(session_counter_api),
    meck:unload(application_obj),
    meck:unload(session_counter_api).
