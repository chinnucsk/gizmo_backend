...
test_create_ok(_) ->
    ...
    meck:expect(application_obj, create, fun(_) -> {ok, <<"key">>} end),
    ...
    [{_, {application_obj, create,[[]]}, {ok, <<"key">>}}] = meck:history(application_obj),
    meck:validate(application_obj),
    meck:unload(application_obj).

test_create_error(_) ->
    meck:new(application_obj),
    meck:expect(application_obj, create, fun(_) -> {error, reason} end),
    ...
    [{_, {application_obj, create,[[]]}, {error, reason}}] = meck:history(application_obj),
    meck:validate(application_obj),
    meck:unload(application_obj).
